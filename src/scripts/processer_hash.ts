...
export default class Processer {
   ...
   * embedding哈希表
   */
  wordHash: { [key: string]: Set<string> }
  wordHashList: string[]
  /**
   * 词转移矩阵
   */
  ...
  async init() {
    ...
    for (const p of datasetsList) {
      ...
      .then(content => {
        ...
        const sentences = preprocessSentence(content)
          .split(/\s/)
          .map(s => {
            const tags = jieba.tag(s)  // 拆分每句为词，哈希值为词性+关键词评分*100取整
            const key = jieba.extract(s, s.length)
            const hashList = tags.map(r => {
              let hash = r.tag
              for (let i = 0; i < key.length; i++) {
                if (r.word === key[i].word) {
                  hash += Math.round(key[i].weight * 100)
                  break
                }
              }
              if (!this.wordHash[hash]) this.wordHash[hash] = new Set()
              this.wordHash[hash].add(r.word)
              return hash
            })
            return hashList
          })
    ...
    this.wordHashList = Object.keys(this.wordHash)
  }
  ...
  train() {
    ...
    for (const article of this.articles) {
      for (const sentence of article) {
        ...
        const firstIndex = this.wordHashList.indexOf(firstWord)
        this.transferMat[0][firstIndex]++
        // 学习转移概率
        for (let i = 0; i < sentence.length; i++) {
          const i_a = this.wordHashList.indexOf(sentence[i])  // 本词的embedding索引
          const i_b = i < sentence.length - 1
            ? this.wordHashList.indexOf(sentence[i + 1])  // 下一个词的embedding索引
      ...
      for (let i = 0; i < article.length; i++) {
        const i_a = this.wordHashList.indexOf(article[i][0])  // 本句首的embedding索引
        const i_b = i < article.length - 1
          ? this.wordHashList.indexOf(article[i + 1][0])  // 下一个句首的embedding索引
          : 0  // 若本句是结尾，则下一个词是结束符号
  ...
  async save() {
    const json = {
      wordHash: (() => {
        const wordHash: { [hash: string]: string[] } = {}
        this.wordHashList.forEach(h => {
          const list: string[] = []
          this.wordHash[h].forEach(w => list.push(w))
          wordHash[h] = list
        })
        return wordHash
      })(),
      wordHashList: this.wordHashList,
    ...
    await writeFile('./src/params/index_hash.ts', 'export const params = ' + newString)
    .then(() => {
      console.log('参数保存成功！')
    })
    .catch(err => {
